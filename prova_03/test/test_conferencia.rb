...
require './prova_03/app/conferencia'
    ...
    class TestConferencia < Minitest::Test
        def test_planejamento_conferencia

            planejamento = Planejamento.new("./prova_03/proposals.txt")
            ...
            assert_equal planejamento.lista_palestra , result_lines_planejamento
        end
            def test_conf_temp
                conf = Conf.new('./prova_03/proposals.txt')
                expected_min = [60, 45, 30, 45, 45, 5, 60, 45, 30, 30, 45, 60, 60, 45, 30, 30, 60, 30, 30]
    
                assert_equal conf.list_temp_palestras, expected_min
            end
    
        def test_hora
            confe = Conf.new('./prova_03/proposals.txt')
            expected_temp = ["9:00"]
    
            assert_equal expected_hora, conf.lista_hora
        end
    end